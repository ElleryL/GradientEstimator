...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        ...
        eps = torch.distributions.Normal(mu,std).sample((self.k,)) # (k, batch_size, z_dim)
        return mu + eps * std
    ...
    def neg_elbo_iwae(self,x,x_prime,z,mu,logvar,cond_distr="bernoulli"):
        ...
        std = torch.exp(0.5 * logvar)
        log_q_z_x = gaussian_log_prob(mu,std,z.detach()).sum(-1) #(K,batch_size)
    ...
    def neg_elbo_dreg(self,x,x_prime,z,mu,logvar,cond_distr="bernoulli"):
        '''
